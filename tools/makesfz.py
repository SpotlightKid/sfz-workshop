...
def get_root_note(
    ...
    if root_note is None:
        acc = sample_info.get("accidental") or ""
        acc = {"es": "b", "is": "#"}.get(acc, acc)
        note = sample_info.get("basenote", "").lower()
        ...
        if note:
            root_note = note_name_to_number(
                note + acc + (sample_info.get("octave") or ""), base_octave
            )

    if root_note is None and detect_pitch:
        root_note = estimate_root_note(str(path), start=50)
...
def main(args=None):
    ...
    for path in sorted(find_samples(args.sampledir, args.file_types)):
        ...
        log.debug(f"File: {path.stem} info: ${info}")
